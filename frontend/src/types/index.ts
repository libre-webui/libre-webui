...
export type AutomationTrigger =
   ...
   * triggers have no schedule, so they never produce a next-run time.
   */
 ...
 * model and generation parameters. Mirrors the backend shape (snake_case).
 * Every id is revalidated against the invoking user's effective permissions
 * when the persona is used, never at bind time alone.
 */
