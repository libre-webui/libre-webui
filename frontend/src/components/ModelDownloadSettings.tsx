...
type ModelDownloadMode = 'admins' | 'all-users';

/**
 * Administrator control over who may pull models (Ollama and the Hugging
 * Face browser, which downloads through Ollama). The backend enforces the
...
export const ModelDownloadSettings: React.FC = () => {
  ...
  const handleChange = async (checked: boolean) => {
    const next: ModelDownloadMode = checked ? 'all-users' : 'admins';
    setSaving(true);
    try {
      const response = await ollamaApi.setModelAccess(next);
      if (!response.success) {
        throw new Error('Model download access update failed.');
      }
      setMode(next);
      toast.success(t('userManager.modelAccess.saved'));
    } catch {
      toast.error(t('userManager.modelAccess.saveFailed'));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className='flex items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
            {t('userManager.modelAccess.description')}
          </p>
        </div>
