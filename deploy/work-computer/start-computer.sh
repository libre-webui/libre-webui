...
mkdir -p "$STATE_DIR" "$PROFILE_DIR" "$XDG_CACHE_HOME"
...
# Bring the view-only/control surface up while the independent browser and
# audio stacks initialize. The first VNC frame can show the desktop wallpaper;
# Chromium replaces it as soon as its maximized window is ready. The first
# password grants control; the second remains view-only, and raw VNC stays on
# container loopback behind the authenticated WebSocket relay.
(
...
# Audio does not depend on Chromium. Prepare it concurrently with browser
# startup, then join it before reporting the complete computer session ready.
(
...
wait "$AUDIO_SETUP_PID"
wait "$SCREEN_SETUP_PID"
