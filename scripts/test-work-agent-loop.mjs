...
test('native DSH reasoning accompanies restored tool turns only for the same native route', () => {
  const provider = {
    providerType: 'dsh',
  ...
  const rows = [
        ...
        workProviderState: {
          ...
          providerMetadata: {
            nativeDsh: {
              ...
              replayState: {
                response: { signature: 'opaque-native-signature' },
              },
            },
          },
          thinking: 'Reasoning required to continue this native tool step.',
          toolCalls: [
            {
              id: 'native-call',
              name: 'read_file',
    ...
    },
    {
      role: 'tool',
      kind: 'tool_result',
      content: 'The workspace plan.',
      metadata: { name: 'read_file', toolCallId: 'native-call' },
    },
  ];
  const restored = restorePersistedWorkContext(
    rows,
  ...
  assert.equal(
    restored[0].thinking,
    rows[0].metadata.workProviderState.thinking
  );
  assert.equal(restored[1].tool_call_id, 'native-call');
  const terminal = {
    ...rows[0],
    kind: 'message',
    content: 'Completed native turn',
    metadata: {
      workProviderState: {
        ...rows[0].metadata.workProviderState,
        toolCalls: [],
      },
    },
  };
  assert.deepEqual(
    ...
    rows[0].metadata.workProviderState.providerMetadata
  );
  assert.deepEqual(
    restored[0].providerMetadata,
    rows[0].metadata.workProviderState.providerMetadata
  );
  assert.equal(
    restorePersistedWorkContext(rows, provider, undefined, 'other-admin')[0]
      ?.providerMetadata,
    undefined
  );
  assert.equal(
  ...
  for (const changed of [
    ...
    { ...provider, providerId: 'other-native-route' },
    { ...provider, model: 'pro' },
  ])
    assert.equal(
      restorePersistedWorkContext(rows, changed, undefined, 'native-admin')[0]
        ?.thinking,
      undefined
    );
});
