...
test('native DSH capabilities and Work creation retain the native provider type and raw model', async t => {
  const [
    { nativeDshProviderService },
    { default: providers },
    { default: runtime },
  ] = await Promise.all([
    distModule('cordis/dsh/native-provider-client.js'),
    distModule('services/workModelProviderService.js'),
    distModule('services/workRuntimeService.js'),
  ]);
  const native = {
    providerId: 'native-deepseek',
    model: 'flash',
    name: 'Flash',
    providerName: 'Native DeepSeek',
  };
  const actors = [];
  t.mock.method(nativeDshProviderService, 'catalog', async userId => {
    actors.push(userId);
    return { status: 'ready', models: [native] };
  });
  const checks = [];
  t.mock.method(
    nativeDshProviderService,
    'assertModel',
    async (providerId, model, userId) => {
      checks.push({ providerId, model, userId });
      return native;
    }
  );
  t.mock.method(providers, 'availability', async () => ({
    ollamaAvailable: false,
    pluginAvailable: false,
  }));
  t.mock.method(runtime, 'isRuntimeAvailable', async () => true);
  const capabilities = await get('/capabilities', 'runs-owner');
  assert.equal(capabilities.status, 200);
  assert.equal(
    capabilities.body.data.available,
    true,
    'native-only model availability is sufficient for Work'
  );
  assert.equal(capabilities.body.data.ollamaAvailable, false);
  assert.equal(capabilities.body.data.pluginAvailable, false);
  assert.deepEqual(capabilities.body.data.nativeDsh, {
    status: 'ready',
    models: [
      {
        model: 'flash',
        ...
        key: 'dsh:native-deepseek:flash',
        label: 'Flash · Native DeepSeek',
        remote: true,
      },
    ],
  });
  assert.deepEqual(actors, ['runs-owner']);
  const response = await fetch(`${baseUrl}/tasks`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${tokenFor('runs-owner')}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      message: 'Use the configured native model.',
      model: native.model,
      providerType: 'dsh',
      providerId: native.providerId,
    }),
  ...
  for (const record of [body.data, body.data.activeRun]) {
    assert.equal(record.model, native.model);
    assert.equal(record.providerType, 'dsh');
    assert.equal(record.providerId, native.providerId);
  }
  ...
  assert.equal(
    checks.length,
    1,
    'an incomplete native route is rejected before provider access'
  );
});
