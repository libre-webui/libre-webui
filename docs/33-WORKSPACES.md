...
| Current Kubernetes/Helm deployment        | Experimental: `WORK_RUNTIME_BACKEND=kubernetes` runs sandboxes as Pods with PVC workspaces (runs, files, commands, git). Preview and terminals are not yet supported, and the Helm chart does not yet create the namespace, RBAC, or NetworkPolicies — see `WORK_KUBERNETES_DRIVER_PLAN.md`. | Unavailable.                                                                  |
