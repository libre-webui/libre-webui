...
| Current Kubernetes/Helm deployment        | Experimental: `WORK_RUNTIME_BACKEND=kubernetes` runs sandboxes as Pods with PVC workspaces (runs, files, commands, git, interactive terminals). The Helm chart does not yet create the namespace, RBAC, or NetworkPolicies — see `WORK_KUBERNETES_DRIVER_PLAN.md`. | Supported when the backend runs in-cluster: the signed proxy targets the sandbox Pod IP directly. |
