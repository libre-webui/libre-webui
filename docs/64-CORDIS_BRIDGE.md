...
Install its standalone Apache-2.0 package from
[libre-webui/dsh-native-provider](https://github.com/libre-webui/dsh-native-provider),
or prepare a bundle from the Libre WebUI distribution. Both use the package name
`@libre-webui/dsh-native-provider` and keep provider keys in native DSH. The
connection requires the same Unix host and OS account, with a private Unix
socket; it cannot isolate applications that share that account. It exposes
model inference only, without native agent sessions or native tool execution.
See the configuration guide for installation, profile restarts, upgrades and
removal. Missing connections or selected models fail without changing providers.
