...
| `WORK_STATUS_BLURB_MODEL`             | `1`                                                                                           | Set `0` to skip the one model request that writes an agent's sidebar status line after a run                |
...
| `WORK_PREVIEW_BIND`                   | `127.0.0.1`                                                                                   | Host interface a task preview port is published on; native Docker Engine Compose deployments must use a reachable non-public bridge interface |
| `WORK_DOCKER_PUBLISHED_HOST`          | app default: same as `WORK_PREVIEW_BIND`; Compose default: `host.docker.internal`              | Backend-facing host/IP for Docker-published preview, screen, and audio ports                                |
| `WORK_COMPUTER_SCREEN_PORT`           | `6080`                                                                                        | Container port of the Work Computer screen bridge (websockify) in GUI-enabled sandboxes                     |
| `WORK_COMPUTER_AUDIO_PORT`            | `6081`                                                                                        | Container port of the Work Computer audio bridge (websockify → PulseAudio monitor) in GUI-enabled sandboxes |
