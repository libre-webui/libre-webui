...
| Chat                              | `/chat`<br />`/c/:sessionId` |             24 |                   1 | `backend/src/types/index.ts`                                                                                 | `scripts/capability-contracts.json`                                                                                               | `backend/src/services/preferencesService.ts`                                                                               | `docs/01-QUICK_START.md`                                     | `ChatRequestService prepares target, persona prompt, and shared messages`<br />`scripts/test-chat-context.mjs` (`test:package`)<br />`oversized history compacts into one summary on a user-turn boundary`<br />`scripts/test-context-compaction.mjs` (`test:package`)                                                                                                                                                                                                                                                         |
...
| OpenAI-compatible API             | —                            |              2 |                   0 | `backend/src/routes/openaiCompat.ts`                                                                         | N/A — The /v1 surface is built into the server and authenticated with personal API keys rather than installed through a manifest. | `backend/src/services/apiTokenService.ts`                                                                                  | `docs/47-OPENAI_COMPATIBLE_API.md`                           | `non-streaming completion returns an OpenAI chat.completion with usage`<br />`scripts/test-openai-compat.mjs` (`test:package`)<br />`lwk_ tokens are scope-checked: chat passes, notes is rejected`<br />`scripts/test-openai-compat.mjs` (`test:package`)                                                                                                                                                                                                                                                                     |
