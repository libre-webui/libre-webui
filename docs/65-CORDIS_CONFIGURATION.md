...
The optional [dsh-native-provider](https://github.com/libre-webui/dsh-native-provider)
plugin exposes the models and provider connections already configured in another
DSH instance, such as the local web app on port 3080. Install it into that
instance's existing profile. It calls only `ctx.llm`:
...
owned by that account and mode `0700`, and socket mode `0600`. It adds no TCP
...
The OS account is the local access boundary: other processes running as that
account can use its socket. This connection does not provide separate native
provider credentials or isolation between applications sharing that account.

### Install the standalone plugin

In **DSH → Plugins → Add plugin**, paste this public repository URL into
**Package name or address**, then click **Install**:

```text
https://github.com/libre-webui/dsh-native-provider
```

Enable the component when DSH asks. The public package is version `0.1.1`,
licensed under Apache-2.0, and includes its prebuilt runtime and bundle patch.
It requires no local build, install scripts, or npm runtime dependencies. The
bare package name `@libre-webui/dsh-native-provider` is not published to npm;
use the GitHub URL in the dialog.

The bundle chooses `<DSH home>/lwui-provider/llm.sock`, normally
`$HOME/.dsh/lwui-provider/llm.sock`. A configured `DSH_HOME` takes precedence.
The private socket directory is created if missing. Use one active bridge per
DSH home, or override the socket path in the profile's user `cordis.patch.yml`
for additional profiles. The full path must fit within 100 UTF-8 bytes and have
no symbolic-link components. See the standalone configuration guide for overrides.
...
Restart that profile after CLI installation; a live UI installation can enable
the plugin immediately. Follow any restart notice DSH displays. No native DSH
source changes or copies of provider keys are required.

### Prepare a bundle from Libre WebUI

Libre WebUI also ships a preparation script. From a source checkout, build the
backend and prepare a fresh output directory:
...
script; run the last two commands from that installation directory without the
build step. Restart the selected DSH profile afterwards. Both preparation paths
refuse existing output directories and include package metadata, the license,
and installation notes.

### Connect Libre WebUI

Point Libre WebUI's `cordis.config.yml` at the same absolute socket path. For
the public bundle's default, replace `/absolute/home` with your actual home:
...
Finish or cancel active native requests before changing the plugin. To replace
the old local `0.0.0`/`0.1.0` bundle from DSH's UI, use **Uninstall**, return to
**Add plugin**, and install the public GitHub URL above. Retain any custom socket
path through a supported user profile override. Native sessions and credentials
are preserved.

For an installation already sourced from GitHub, the CLI can update it:

```bash
dsh plugin --profile web update @libre-webui/dsh-native-provider
```

Restart after CLI updates and verify the version. A previously disabled plugin
remains disabled; check its enabled state before testing the connection. To pin
or roll back, use `github:libre-webui/dsh-native-provider#<commit>` as the source.
For custom locally prepared bundles, build a new output directory and re-add
that directory instead; updating a local dependency does not pull GitHub.

To remove the connection, first remove LWUI's `nativeProvider.socketPath` setting
or set `LIBRE_DSH_PROVIDER_SOCKET` to an empty value, then run:

```bash
dsh plugin --profile web remove @libre-webui/dsh-native-provider
```

Restart the DSH profile after removal. Saved LWUI tasks remain, but their native
model requests fail until the same explicit connection is restored. Removing
this plugin does not delete DSH's configured providers or credentials. Delete
old generated bundle directories only after they are no longer installed.
