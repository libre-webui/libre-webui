...
app.use(
  helmet({
    // Work previews are generated by arbitrary local dev servers, which do
    // not normally emit CORP. Their iframe is still constrained to loopback.
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        frameAncestors: ["'self'"],
        formAction: ["'self'"],
        // npx desktop mode is production but previews are loopback HTTP.
        // Upgrading them to HTTPS would make every generated preview fail.
