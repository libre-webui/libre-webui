...
const AGENT_IDENTITY_REQUIRED_SCHEMA = {
...
const AGENT_SEEN_REQUIRED_SCHEMA = {
...
const collectMissingAgentIdentitySchema = (
  database: Database.Database
...
const MIGRATIONS: readonly SQLiteMigration[] = [
  ...
  },
  {
    version: 25,
    name: 'agent-identity',
    checksum: AGENT_IDENTITY_MIGRATION_CHECKSUM,
    apply(database) {
      addColumnIfMissing(database, 'work_tasks', 'persona_id', 'TEXT');
      addColumnIfMissing(database, 'work_tasks', 'status_blurb', 'TEXT');
      addColumnIfMissing(database, 'work_tasks', 'is_agent', 'INTEGER');
      const missing = collectMissingAgentIdentitySchema(database);
      if (missing.length > 0) {
        throw new Error(
  ...
  },
  {
    version: 27,
    name: 'agent-seen',
    checksum: AGENT_SEEN_MIGRATION_CHECKSUM,
    apply(database) {
      addColumnIfMissing(database, 'work_tasks', 'last_seen_at', 'INTEGER');
      const missing = collectMissingAgentSeenSchema(database);
      if (missing.length > 0) {
        throw new Error(
